...
pause() fully releases the camera (cap.release()) so the trainer
can open it. resume() re-opens it. This avoids the two-process
camera conflict on Windows.
"""
...
class CameraThread(threading.Thread):
    ...
    # ── Pause / Resume — fully releases camera so trainer can use it ──────
    ...
    def run(self):
        ...
        while not self._stop_event.is_set():
            ...
            # ── Paused: release camera and wait ───────────────────────────
            if self.is_paused():
                if cap is not None:
                    cap.release()
                    cap = None
