...
class CameraThread(threading.Thread):
    ...
    def __init__(self, gesture_queue: GestureQueue, settings: Settings,
                 adaptive_engine=None, presentation_mode=None):
        super().__init__(name="CameraThread")
        self.gesture_queue    = gesture_queue
        self.settings         = settings
        self._adaptive        = adaptive_engine
        self._presentation    = presentation_mode   # Phase 5
    ...
    def _process_gesture_mode(self, results, frame):
        ...
        # ── Presentation mode intercept (Phase 5) ─────────────────────────
        # When active, presentation mode handles ALL gesture processing.
        # Normal gesture pipeline is bypassed entirely.
        if self._presentation and self._presentation.active:
            if hand_detected:
                ...
                self._presentation.process_frame(
                ...
                if self.settings.show_debug_window:
                    ...
                    cv2.putText(frame, f"PRES: {gesture_name} {confidence:.2f}",
            ...
            return   # skip normal gesture pipeline
